...
#----------------------------------------------------------------------
    ...
    def move_to_position(self):
        ...
        print('exiting move_to_position in GUI')

    def wait_for_motion_complete(self):

        timeout = time.time() + 300
        print('entering wait for motion complete')
        while True:
            # Stop the motor if STOPNOW Button is hit
            if self.STOPNOW is True:
                self.mc.stop_now()
                print('The motor unexpectedly stopped upon hitting the STOP NOW button')
                break

            else:
                x_not_moving, y_not_moving = self.mc.check_not_moving()

                if x_not_moving and y_not_moving:
                    break
                elif time.time() > timeout:
                    raise TimeoutError('Motor has been moving for over 5 min?')
